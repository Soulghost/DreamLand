...
void SGRoundDispatcher::nextAction() {
    ...
    switch (action->type) {
        case SGPlayerActionTypeCommonAttack: {
            SGLog::info("%s 对 %s进行物理攻击", caller->_player->name.c_str(), targets.at(0)->_player->name.c_str());
            // 处于禁止状态的角色无法行动
        ...
        case SGPlayerActionTypeMagicSkill: {
            SGLog::info("%s 对 %s进行法术攻击", caller->_player->name.c_str(), targets.at(0)->_player->name.c_str());
            // 处于禁止状态的角色无法行动
        ...
        case SGPlayerActionTypeProtect: {
            DragonBaseModel *protecter = caller;
            for (DragonBaseModel *protectedOne : targets) {
                if (protectedOne == protecter) continue;
                if (protectedMap.find(protectedOne) != protectedMap.end()) {
                    Vector<DragonBaseModel *> protecters = protectedMap.at(protectedOne);
                    protecters.pushBack(protecter);
                } else {
                    Vector<DragonBaseModel *> protecters{caller};
                    protectedMap[protectedOne] = protecters;
                }
                SGLog::info("%s 对 %s进行保护", protecter->_player->name.c_str(), protectedOne->_player->name.c_str());
            }
            actionIdx++;
            nextAction();
            break;
