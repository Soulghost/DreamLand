...
SGPlayer* DragonFightScene::createDemoPlayer(const string &name, float scale) {
    ...
    p->name = name;
    p->hp = 200 + scale * 6 * 5 + 10 * scale;
    p->hpmax = p->hp;
    p->mp = p->mpmax = 1000;
    p->pl = 10 + scale * 1 * 5 + 5.6f * scale;
    p->ph = 15 + scale * 1.5f * 5 + 6.0f * scale;
    p->ml = 5 + scale * 0.8f * 5 + 5.5f * scale;
    p->mh = 5 + scale * 1.0f * 5 + 5.5f * scale;
    p->speed = 10 + scale;
    p->pd = 5 + scale * 1.2 + 5 * scale;
    p->pcrit = 20;
    return p;
}
...
void DragonFightScene::commonInit() {
    ...
    Vector<SGPlayer *> leftRolePlayers {
        createDemoPlayer("血骑士", baseLv + 50),
        createDemoPlayer("魔枪兵", baseLv),
        createDemoPlayer("地狱骑士", baseLv + 10)
    };
    ...
    Vector<SGPlayer *> leftPetPlayers {
        createDemoPlayer("炎魔2号", baseLv),
        createDemoPlayer("炎魔3号", baseLv),
        createDemoPlayer("冰雪企鹅", baseLv + 30)
    };
    ...
    Vector<SGPlayer *> rightRolePlayers {
        createDemoPlayer("白龙", baseLv + 100),
        createDemoPlayer("魔灵", baseLv),
        createDemoPlayer("天师", baseLv)
    };
    ...
    Vector<SGPlayer *> rightPetPlayers {
        createDemoPlayer("兽人", baseLv),
        createDemoPlayer("魔兽", baseLv),
        createDemoPlayer("蓝龙", baseLv)
    };
    ...
    SGRoundDispatcher::getInstance()->setActionReducer([this](SGPlayerAction *action, ActionPromise actionPromise) {
        ...
        if (role->_player->name == "白龙") {
            action->type = SGPlayerActionTypeProtect;
            action->progress = SGPlayerActionProgressCommitted;
            Vector<DragonBaseModel *> protectedOnes = action->caller->getModelPosition() == ModelPositionLeft ? SGRoundDispatcher::getInstance()->_leftRoles : SGRoundDispatcher::getInstance()->_rightRoles;
            action->targets = protectedOnes;
            _actionPromise(action);
            return;
        }
        ...
        if (role->_player->name == "冰雪企鹅") {
            action->type = SGPlayerActionTypePhysicalSkill;
            action->name = "jianqisishe";
            action->progress = SGPlayerActionProgressCommitted;
            DragonBaseModel *randomTarget = action->caller->getModelPosition() == ModelPositionRight ? SGRoundDispatcher::getInstance()->_leftRoles.getRandomObject() : SGRoundDispatcher::getInstance()->_rightRoles.getRandomObject();
            action->targets.pushBack(randomTarget);
            _actionPromise(action);
